...
import { UserAnalytics, AddressInteraction } from "@/lib/analytics";
...
export function AddressInteractionChart({ analytics, privacyMode }: AddressInteractionChartProps) {
  ...
  const formatAddress = (address: string) => {
    if (privacyMode && !address.startsWith("0x1::")) {
      return "••••••••••••";
    }
  ...
  const getAddressLabel = (address: string) => {
    if (address.includes("aptos_coin")) return "Aptos Coin";
    if (address.includes("framework")) return "Aptos Framework";
    if (address === config.moduleAddress) return "FlowShield";
    return "Unknown";
  };

  const openInExplorer = (address: string) => {
    if (privacyMode || address.includes("::")) return;
    
    const explorerUrl = config.aptosNetwork === 'mainnet' 
      ? `https://explorer.aptoslabs.com/account/${address}`
      : `https://explorer.aptoslabs.com/account/${address}?network=${config.aptosNetwork}`;
    window.open(explorerUrl, '_blank');
  };

  // Mock interaction data for demonstration since we can't expose real addresses
  const mockInteractions: AddressInteraction[] = privacyMode ? [
    {
    ...
    },
    {
      ...
      transactionCount: analytics.privacyStats.totalDeposits,
      totalValue: analytics.privacyStats.totalMixed,
      lastInteraction: new Date(Date.now() - analytics.privacyStats.daysSinceLastActivity * 24 * 60 * 60 * 1000).toISOString(),
      label: "FlowShield"
    }
  ] : walletStats.topInteractions;
    ...
    <div className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Address interactions are anonymized. Only system contracts and FlowShield interactions are shown.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {privacyMode 
              ? "System contracts and FlowShield interactions only"
              ...
              mockInteractions.map((interaction, index) => (
                ...
                >
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {interaction.address === config.moduleAddress && (
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {formatValue(interaction.totalValue)} APT
                        </div>
                        <div className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!privacyMode && !interaction.address.includes("::") && (
                      <Button
                        variant="ghost"
                        size="sm"
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {privacyMode ? "••••" : formatValue(walletStats.averageTransactionValue)} APT
                  </span>
                </div>
              </div>
            </div>
