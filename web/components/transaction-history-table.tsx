...
export function TransactionHistoryTable({ address, privacyMode }: TransactionHistoryTableProps) {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      const processedTransactions: TransactionData[] = txns
        .filter(tx => tx.type === 'user_transaction') // Only process user transactions
        .map((tx: any) => {
          const isFlowShield = tx.payload?.function?.includes(config.moduleAddress);
          let type: TransactionData['type'] = 'other';
          let amount = 0;
          let poolDenomination: number | undefined;
          
          if (isFlowShield) {
            if (tx.payload?.function?.includes('deposit')) {
              type = 'deposit';
              amount = tx.payload?.arguments?.[1] 
                ? parseInt(tx.payload.arguments[1]) / 100000000 
                : 0;
              poolDenomination = amount;
          ...
          return {
            ...
            isFlowShield,
            counterparty: privacyMode && isFlowShield ? "Hidden" : tx.payload?.arguments?.[2],
            poolDenomination
          };
        });
  ...
  const filterTransactions = () => {
    ...
    if (privacyMode) {
      ...
      filtered = filtered.map(tx => ({
        ...
        amount: tx.isFlowShield ? tx.amount : 0 // Show FlowShield amounts but hide others
      }));
    }
  ...
  const formatAmount = (amount: number, isPrivate: boolean = false) => {
    ...
    return amount > 0 ? `${amount.toFixed(4)} APT` : "-";
  };
  ...
  const openInExplorer = (hash: string) => {
    const explorerUrl = config.aptosNetwork === 'mainnet' 
      ...
      : `https://explorer.aptoslabs.com/txn/${hash}?network=${config.aptosNetwork}`;
