...
export default function AnalyticsPage() {
  ...
  const loadAnalytics = async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchUserAnalytics(account.address.toString());
      setAnalytics(data);
    } catch (error) {
                ...
                <div className="space-y-4">
                  {analytics.privacyStats.poolUsage.map((pool) => (
                    <div key={pool.denomination} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Badge variant="outline">{pool.denomination} APT</Badge>
